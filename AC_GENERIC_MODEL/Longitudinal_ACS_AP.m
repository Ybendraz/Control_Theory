...
%%
[V_long,D_long,W_long] = eig(sysLON0.A);
w_1 = abs(D_long(1,1));
w_2 = abs(D_long(3,3));
...
%----------------------------------------------------------------
V_abs = abs(V_long);
State_mod_map = zeros(size(sysLON0.A,1),2);
for i=1:size(sysLON0.A,1)
...
ol_integral = linmod('ACS_LON_AP_MODAL_INTEGRAL');
...
K_long1_y= real(-VW(n+1:n+m,:)/(ol_integral.c*VW(1:n,:)+ol_integral.d*VW(n+1:n+m,:)));
...
H_long_1 = eye(2);
cl_integral = linmod('ACS_LONG_AP_MODAL_CL');
cl_integral = ss(cl_integral.a,cl_integral.b,cl_integral.c,cl_integral.d);
damp(cl_integral);
H_long_1 = inv(dcgain(cl_integral));
%%
[A,B,C,D]=linmod('ACS_LONG_AP_MODAL_CL');  % linéarisation autour de x0
damp(A)
%%
step(ss(A,B,C,D),50)
