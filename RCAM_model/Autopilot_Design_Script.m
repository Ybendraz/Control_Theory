...
RCAM_OP1_LONG.OutputName = {'u','W','q','theta'};
...
% phugoïd frequency/damping : w = 2.21 rad/s, xi = 0.483
% short mode frequency/ damping : w = 0.12 rad/s, xi = 0.13

% Desired phugoïd frequency/damping: w = 1.2*2.21 rad/s, xi_ideal = 0.7
% Desired short mode frequency/ damping: w = 1.2*0.12 rad/s, xi_min = 0.5
%% Longitudinal autopilot design:
% Plotting open loop poles and zeros:
%zpk(RCAM_OP1_LONG)
plot(eig(RCAM_OP1_LONG),'x','LineWidth',2)
grid on
xlabel("Re")
ylabel("Im")

% the pitch is only impacted by the elevator input, we can start by setting
% the q, theta loop:
G_q_dT = tf(RCAM_OP1_LONG(4,2));
%
%% Lateral model
% idx_lat_inputs = [1 3];
